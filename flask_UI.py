...
def _build_connection_string(database_override: str | None = None) -> tuple[str, str]:
    db_driver = (os.getenv("DB_DRIVER") or "ODBC Driver 17 for SQL Server").strip()
    db_server = (os.getenv("DB_SERVER") or "MSI").strip()
    db_port = (os.getenv("DB_PORT") or "").strip()
    db_name = (database_override or os.getenv("DB_NAME") or "ChatbotDB").strip()
    db_user = (os.getenv("DB_USER") or "").strip()
    db_password = os.getenv("DB_PASSWORD") or ""

    use_trusted_connection = _env_flag(
        "DB_TRUSTED_CONNECTION",
        default=True,
    )
    encrypt = "yes" if _env_flag("DB_ENCRYPT", default=True) else "no"
    trust_server_certificate = "yes" if _env_flag("DB_TRUST_SERVER_CERTIFICATE", default=True) else "no"
    connection_timeout = int((os.getenv("DB_CONN_TIMEOUT") or "30").strip())
    ...
    if use_trusted_connection:
        ...
        if not db_user or not db_password:
            raise RuntimeError(
                "Missing DB_USER/DB_PASSWORD while DB_TRUSTED_CONNECTION is disabled."
            )
...
def _ensure_database_exists():
    global _db_ensured
    if _db_ensured or not _env_flag("DB_AUTO_CREATE", default=True):
        return

    target_db = (os.getenv("DB_NAME") or "ChatbotDB").strip()
    if not target_db or target_db.lower() == "master":
        _db_ensured = True
        return
    ...
    with _db_ensure_lock:
        ...
        admin_conn_str, _ = _build_connection_string(database_override="master")
        conn = pyodbc.connect(admin_conn_str, autocommit=True)
        cursor = conn.cursor()
        escaped_db_id = target_db.replace("'", "''")
        safe_db_name = target_db.replace("]", "]]")
        cursor.execute(
            f"IF DB_ID(N'{escaped_db_id}') IS NULL CREATE DATABASE [{safe_db_name}]"
        )
        conn.close()
        _db_ensured = True
...
def get_connection():
    conn_str, _ = _build_connection_string()
    try:
